...
const handkerchief = simpleFn(/* wgsl */ `
  (pos: vec2f) -> vec2f {
    let r = length(pos); 
    let theta = atan2(pos.y, pos.x);
...
const hyperbolic = simpleFn(/* wgsl */ `
  (pos: vec2f) -> vec2f {
    let r = length(pos);
    let theta = atan2(pos.y, pos.x);
